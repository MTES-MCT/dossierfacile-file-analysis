...
class AmqpService:
    ...
    def _message_callback(self, channel, method_frame, properties, body):
        ...
        def _ack():
            try:
                ...
                if channel and getattr(channel, "is_open", False):
                    ...
                    logger.warning("Channel is closed when trying to ack; skipping ack (message will be re-queued by broker).")
        ...
        def _retry_message():
            try:
                if not (self.connection and getattr(self.connection, "is_open", False) and channel and getattr(channel, "is_open", False)):
        ...
        def _on_done(future):
            try:
                ...
                logger.info(f"Analysis already exists for file_id {e.file_id}; acknowledging message without retry")
